...
from GPT_SoVITS.configs.config import change_choices, get_weights_names, name2gpt_path, name2sovits_path

SoVITS_names, GPT_names = get_weights_names()
from GPT_SoVITS.configs.config import pretrained_sovits_name
...
dict_language_v1 = {
    ...
    ("Chinese-English Mixed"): "zh",  # 按中英混合识别####不变
    ("Japanese-English Mixed"): "ja",  # 按日英混合识别####不变
...
dict_language_v2 = {
    ...
    ("Japanese"): "all_ja",  # 全部按日文识别
    ("Yue"): "all_yue",  # 全部按中文识别
    ("Korean"): "all_ko",  # 全部按韩文识别
    ("Chinese-English Mixed"): "zh",  # 按中英混合识别####不变
    ("Japanese-English Mixed"): "ja",  # 按日英混合识别####不变
    ("Yue-English Mixed"): "yue",  # 按粤英混合识别####不变
    ("Korean-English Mixed"): "ko",  # 按韩英混合识别####不变
    ("Multilingual Mixed"): "auto",  # 多语种启动切分识别语种
...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    if if_lora_v3 == True and is_exist == False:
        info = path_sovits + "SoVITS %s" % model_version + ("Missing Pretrained Model, Cannot Load LoRA Weights")
        gr.Warning(info)
    ...
    if prompt_language is not None and text_language is not None:
        ...
        yield (
            ...
            {"__type__": "update", "value": ("Model is loading, please wait..."), "interactive": False},
        )
    ...
    yield (
        ...
        {"__type__": "update", "value": False, "interactive": True if model_version not in v3v4set else False},
        {"__type__": "update", "visible": True if model_version == "v3" else False},
        {"__type__": "update", "value": ("Start inference"), "interactive": True},
    )
...
def audio_sr(audio, sr):
    ...
    if sr_model == None:
        ...
        try:
            ...
            gr.Warning(("Super-Resolution Model Not Found. Please follow the tutorial to download the model file if you want to use it."))
...
def get_tts_wav(
    ...
    if if_sr == True and opt_sr == 24000:
        print(("Running Audio Super-Resolution"))
...
with gr.Blocks(title="GPT-SoVITS WebUI", analytics_enabled=False, js=js, css=css) as app:
    with gr.Group():
        gr.Markdown(html_center(("Model switch"), "h3"))
        with gr.Row():
            ...
            refresh_button = gr.Button(("refreshing model paths"), variant="primary", scale=14)
            refresh_button.click(fn=change_choices, inputs=[], outputs=[SoVITS_dropdown, GPT_dropdown])
        gr.Markdown(html_center(("*Please upload and fill reference information"), "h3"))
        with gr.Row():
            inp_ref = gr.Audio(label=("Primary Reference Audio (Please upload reference audio within 3-10 seconds, exceeding this limit will cause an error!)"), type="filepath", scale=13)
            with gr.Column(scale=13):
                ref_text_free = gr.Checkbox(
                    label=("Enable no reference mode. If you don't fill 'Text for reference audio', no reference mode will be enabled.")
                    + ("v3 does not support this mode currently, using it will cause an error."),
                    value=False,
                ...
                gr.Markdown(
                    html_left(
                        ("Recommended to use a Finetune-GPT when using Prompt-Free Mode.")
                        + "<br>"
                        + ("If reference audio is not clear or unsure what to write, enable this option to ignore the reference text.")
                    )
                )
                prompt_text = gr.Textbox(label=("Text of Primary Reference Audio"), value="", lines=5, max_lines=5, scale=1)
            with gr.Column(scale=14):
                prompt_language = gr.Dropdown(
                    label=("Language of Primary Reference Audio"),
                ...
                sample_steps = (
                    gr.Radio(
                        label=("Sampling Steps: If feel noisy, try increasing, if feel slow, try decreasing"),
                        value=32 if model_version == "v3" else 8,
                        choices=[4, 8, 16, 32, 64, 128] if model_version == "v3" else [4, 8, 16, 32],
                        visible=True,
                    )
                ...
                if_sr_Checkbox = gr.Checkbox(
                    label=("For V3 model, if generated audio sounds somewhat muffled, try enable audio super-resolution."),
                    value=False,
                    interactive=True,
                    show_label=True,
                    visible=False if model_version != "v3" else True,
                )
        gr.Markdown(html_center(("*Please fill in the target text and language mode for synthesis"), "h3"))
        with gr.Row():
            with gr.Column(scale=13):
                text = gr.Textbox(label=("Inference text"), value="", lines=26, max_lines=26)
            with gr.Column(scale=7):
                text_language = gr.Dropdown(
                    label=("Inference text language") + (".Less Multilingual is better"),
                    choices=list(dict_language.keys()),
                    value=("Chinese"),
                    scale=1,
                )
                ...
                gr.Markdown(value=html_center(("Adjust speech rate, higher for faster")))
                if_freeze = gr.Checkbox(
                    label=("Adjust the speech rate and tone of the last synthesis result to prevent randomness."),
                    value=False,
                    interactive=True,
                    show_label=True,
                    scale=1,
                ...
                with gr.Row():
                    speed = gr.Slider(
                        minimum=0.6, maximum=1.65, step=0.05, label=("Speech rate"), value=1, interactive=True, scale=1
                    )
                ...
                gr.Markdown(html_center(("GPT sampling parameters (not too low when there's no reference text. Use default if unsure)：")))
                top_k = gr.Slider(
                    minimum=1, maximum=100, step=1, label=("top_k"), value=15, interactive=True, scale=1
                )
                top_p = gr.Slider(
                    minimum=0, maximum=1, step=0.05, label=("top_p"), value=1, interactive=True, scale=1
                )
                temperature = gr.Slider(
                    minimum=0, maximum=1, step=0.05, label=("temperature"), value=1, interactive=True, scale=1
                )
        with gr.Row():
            inference_button = gr.Button(value=("Start inference"), variant="primary", size="lg", scale=25)
            output = gr.Audio(label=("Inference Result"), scale=14)
