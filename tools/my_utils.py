...
def check_for_existance(file_list: list = None, is_train=False, is_dataset_processing=False):
    ...
    if is_train == True and file_list:
        file_list.append(os.path.join(file_list[0], "name2text.txt"))
        file_list.append(os.path.join(file_list[0], "tokens-bert"))
        file_list.append(os.path.join(file_list[0], "cn_hubert"))
        file_list.append(os.path.join(file_list[0], "32k_audio"))
        file_list.append(os.path.join(file_list[0], "name2semantic.tsv"))
        ...
        if is_train:
            for file, status in zip(file_list, files_status):
                if status:
                    ...
                    gr.Warning(file)
            gr.Warning(("No Such File or Folder"))
            return False
            ...
            if file_list[0]:
                ...
                gr.Warning(("Expected No Empty Path"))
            return False
    return True
...
def check_details(path_list=None, is_train=False, is_dataset_processing=False):
    if is_dataset_processing:
        ...
        if not list_path.endswith(".list"):
            gr.Warning(("Please Fill in the Correct List Path"))
            return
        if audio_path:
            if not os.path.isdir(audio_path):
                gr.Warning(("Please Fill in the Correct Audio Folder Path"))
                return
        ...
        if os.path.exists(wav_path):
            ...
            gr.Warning(wav_path + ("Path Error"))
        return
    if is_train:
        path_list.append(os.path.join(path_list[0], "name2text.txt"))
        path_list.append(os.path.join(path_list[0], "cn_hubert"))
        path_list.append(os.path.join(path_list[0], "32k_audio"))
        path_list.append(os.path.join(path_list[0], "name2semantic.tsv"))
        phone_path, hubert_path, wav_path, semantic_path = path_list[1:]
        with open(phone_path, "r", encoding="utf-8") as f:
            if f.read(1):
                ...
            else:
                gr.Warning(("Missing Phoneme Dataset"))
        if os.listdir(hubert_path):
            ...
        else:
            gr.Warning(("Missing Hubert Dataset"))
        if os.listdir(wav_path):
            ...
        else:
            gr.Warning(("Missing Audio Dataset"))
        df = pd.read_csv(semantic_path, delimiter="\t", encoding="utf-8")
        if len(df) >= 1:
            ...
        else:
            gr.Warning(("Missing Semantics Dataset"))
